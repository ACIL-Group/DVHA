...
import numpy as np
from nuart.clustering import DualVigilanceFuzzyART
...
description = "Run the Dual Vigilance Fuzzy ART clustering algorithm"


def run_single(inputs, targets, rho_ub, rho_lb, alpha, beta, max_epochs):
    # run DualVigilanceFuzzyART and get labels
    dvfa = DualVigilanceFuzzyART(rho_ub, rho_lb, alpha, beta, max_epochs=max_epochs, shuffle=False)

    timer = timeit.default_timer()
    labels = dvfa.fit(inputs)
    ...
    # return results
    return dvfa.iterations, dvfa.num_clusters, dvfa.num_categories, performance, elapsed
...
def evaluate(args):
    ...
    if args.restore_file is not None:
        ...
        params = [(w, x, y, z) for (w, x), y, z in itertools.product(rho_values, alpha, beta)]
