...
import numpy as np
from nuart.clustering import FuzzyART
...
description = "Run the Fuzzy ART clustering algorithm"


def run_single(inputs, targets, rho, alpha, beta, max_epochs):
    # run FuzzyART and get labels
    fa = FuzzyART(rho, alpha, beta, max_epochs=max_epochs, shuffle=False)
    ...
    # return results
    return fa.iterations, fa.num_clusters, performance, elapsed
...
def run_sweep(inputs, targets, params, max_epochs, enable_mp, num_processes):
    ...
    run_args = [(inputs, targets, *comb, max_epochs) for comb in params]
...
def evaluate(args):
    ...
    if args.restore_file is not None:
        ...
        rho = np.array([args.rho]) if args.rho is not None \
            else np.linspace(args.rho_min, args.rho_max, num=args.rho_count)
        ...
        params = list(itertools.product(rho, alpha, beta))

        logger.info('Running {} combinations of:'.format(len(params)))
        logger.info(' - {} rho values'.format(len(rho)))
    ...
    x.field_names = [
        'Idx', 'Rho', 'Alpha', 'Beta',
        '# Iter', '# Clusters', 'ARI Performance', 'Time'
    ]
    ...
    x.add_row(['Avg'] + ['-'] * 3 + list(avg))

    best_ix = results[:, 2].argmax()
...
def setup_parser(parser):
    ...
    parser.add_argument("--rho", type=float, default=None,
                        help="Rho parameter value to use instead of sweeping for it")
    parser.add_argument("--rho-min", type=float, default=0)
    parser.add_argument("--rho-max", type=float, default=1)
    parser.add_argument("--rho-count", type=int, default=101)
